...
exports.createAppointment = exports.getAppointment = exports.getServices = exports.getDoctorAvailability = exports.getDoctors = void 0;
const data_js_1 = require("../models/data.js");
...
const getDoctors = async (_req, res) => {
    try {
        const result = await db_js_1.pool.query(`SELECT id, first_name, last_name, specialization, degree, experience_years
...
const getDoctorAvailability = async (req, res) => {
    ...
    try {
        const result = await db_js_1.pool.query(`SELECT to_char(appointment_date, 'HH24:MI') AS slot
       FROM appointments
       WHERE doctor_id = $1
         AND appointment_date::date = $2::date
         AND status <> 'Odwołana'`, [id, date]);
        const zajete = new Set(result.rows.map((r) => r.slot));
...
const getServices = (_req, res) => {
    res.json(data_js_1.services);
};
exports.getServices = getServices;
const getAppointment = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db_js_1.pool.query(`SELECT a.id, a.appointment_date, a.type, a.referral_number, a.status,
...
const createAppointment = async (req, res) => {
    const { doctorId, appointmentDate, type, referralNumber, firstName, lastName, pesel, phone, email, } = req.body;
    ...
    try {
        ...
        if (type === 'NFZ' && pesel) {
            ...
            if (existing.rows.length > 0) {
                ...
                await client.query(`UPDATE patients SET first_name = $1, last_name = $2, phone = $3, email = $4 WHERE id = $5`, [firstName, lastName, phone, email, patientId]);
            }
            else {
                const inserted = await client.query(`INSERT INTO patients (first_name, last_name, pesel, phone, email)
           VALUES ($1, $2, $3, $4, $5) RETURNING id`, [firstName, lastName, pesel, phone, email]);
                patientId = inserted.rows[0].id;
            }
        }
        else {
            const inserted = await client.query(`INSERT INTO patients (first_name, last_name, pesel, phone, email)
         VALUES ($1, $2, NULL, $3, $4) RETURNING id`, [firstName, lastName, phone, email]);
            patientId = inserted.rows[0].id;
        }
        const appointment = await client.query(`INSERT INTO appointments (doctor_id, patient_id, appointment_date, type, referral_number, status)
       VALUES ($1, $2, $3::timestamp, $4::appointment_type, $5, 'Zaplanowana')
       RETURNING id, doctor_id, patient_id, appointment_date, type, referral_number, status`, [doctorId, patientId, appointmentDate, type, type === 'NFZ' ? referralNumber || null : null]);
